...
- **Existing Installation Guard in `install.sh`**: Preflight check in `do_install()` detects existing service units or environment files (`/etc/systemd/system/mp3metafix.service`, user units, or `.env` files). It safely stops without modifying production files, protecting customized network bindings, reverse proxy domains, and sandbox permissions from being overwritten. Clearly displays operational commands (`./install.sh --update`, `--access`, `--status`, `--reinstall`). Automatically detects and preserves the active git branch during explicit updates (`./install.sh --update`).
...
### Fixed
- Fixed network inspection and configuration in `scripts/configure_access.py` (`install.sh --access`, `--lan`, `--proxy`, `--domain`) for services utilizing `EnvironmentFile` directives (such as migrated dedicated service accounts). `configure_access.py` now accurately parses referenced environment files and updates them atomically, ensuring network bind and reverse proxy domain settings accurately display and persist.
...
### Added
- Configurable Suno.com AI Metadata Detection & Sync policy (`suno_integration_enabled`): disabled by default for a clean initial state. Administrators can toggle Suno integration in `/admin` (under "AI Services & Metadata Extractors") or via pinned Quick Settings.
- Backend policy enforcement for `POST /api/suno/extract` and `POST /api/suno/apply-artwork`, returning `HTTP 403 Forbidden` when Suno integration is disabled in system settings.
- Frontend UI and auto-detection gating in `/app`: when Suno integration is disabled, embedded Suno tags are not automatically detected on upload, the `✨ Suno Detected` pill is suppressed, and the `✨ Suno Sync` toolbar button is hidden.
- Dynamic "Quick Settings" customization system: administrators can pin/unpin individual configuration settings (`guest_mode_enabled`, `suno_integration_enabled`, `max_sessions`, `max_global_storage_mb`, `session_ttl_minutes`, `max_upload_size_mb`) directly from the Admin Dashboard via "⚡ Quick Settings" pin toggles, dynamically surfacing them in the Quick Settings modal across all app pages.
...
- **Full System Authentication & Access Control Subsystem (`backend/auth.py`, `frontend/js/auth.js`)**:
  ...
  - **Configurable Guest Mode Policy**: Administrator-toggled Guest Mode in Settings allowing unauthenticated visitors to explore the Gateway Hub at `/` and use the mobile-first single-track tagger at `/app`, while restricting MP3MetaManager (`/manager`), live telemetry diagnostics, and server settings to logged-in accounts.
