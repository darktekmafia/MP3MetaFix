> Current status, 2026-09-21: The Opus-in-MP4 fix resolved the M4A HTTP 422. Security fixes and v0.5.1 were pushed in commit `11bbbfb`. Subsequent migration work remains local: the Fedora backend now runs as the dedicated `mp3metafix` account under the enabled system service; the old user service is disabled/inactive. Active data is `/var/lib/mp3metafix`; checkout `data/` is retained recovery data. Use `sudo systemctl restart mp3metafix.service`, not the historical user-service commands below. Read `docs/ACCOUNT_MIGRATION.md` for the failed first attempt, corrected retry, verified successful cutover, rollback, and maintenance limits. The full suite passed 122 tests; post-cutover browser editing is not yet explicitly confirmed. The Ubuntu 24.04 LXC upgraded to v0.5.1, but its account migration remains untested. Guided one-shot account selection/creation and general installer integration remain roadmap work. Everything below is historical context.
