...
- [x] **Existing Installation Guard & Customization Preservation in `install.sh`**:
  ...
  - Warns the operator that an existing installation was detected, safely stopping without modifying production files. Displays command guidance (`./install.sh --update`, `--access`, `--status`, `--reinstall`) to prevent accidental template overwriting of production settings. Automatically detects and preserves the active git branch during explicit updates (`./install.sh --update`).
