...
MP3, WAV, and real Suno Opus M4A editing have been verified. Synthetic AAC M4A tests also pass. The table describes implemented handlers, not complete compatibility certification for every encoder variant.
...
## Local dedicated-account deployment (unreleased)

`scripts/migrate_local_account.py` prepares a system service with selective read-only checkout mounts and private data under `/var/lib/mp3metafix`. It preserves account credentials and configuration, verifies copied data, and provides service/data rollback. A system-manager sandbox probe runs under the dedicated identity before stopping the source; recovery stops restart jobs regardless of active/activating state, and explicit retry archives earlier destination data. `MP3METAFIX_ALLOW_WEB_UPDATES=false` denies updater execution before acquiring a lease or spawning a process; update checks expose the setting for the admin UI. This local developer deployment is distinct from the planned general installer migration. See [account migration](ACCOUNT_MIGRATION.md).

The local cutover has completed: the system service and workers run as `mp3metafix`; the old user service is disabled/inactive. The private data root and effective runtime restrictions were verified alongside a healthy v0.5.1 endpoint. General installation migration remains separate work.

This documentation update changes no runtime interfaces or application behavior. Public installed-version display with update discovery restricted to `/admin` is planned in the [roadmap](../ROADMAP.md), not implemented in v0.5.1.
