...
`scripts/migrate_local_account.py` prepares a system service with selective read-only checkout mounts and private data under `/var/lib/mp3metafix`. It preserves account credentials and configuration, verifies copied data, and provides service/data rollback. `MP3METAFIX_ALLOW_WEB_UPDATES=false` denies updater execution before acquiring a lease or spawning a process; update checks expose the setting for the admin UI. This local developer deployment is distinct from the planned general installer migration. See [account migration](ACCOUNT_MIGRATION.md).
